...
class DefaultPipelineSettingsHelper:
    """Helper class for default pipeline settings."""

    @staticmethod
    def check_stt_engine(
        hass: HomeAssistant, pipeline_language: str, stt_engine_id: str | None
    ...
    def check_tts_engine(
        hass: HomeAssistant, pipeline_language: str, tts_engine_id: str | None
...
def _async_resolve_default_pipeline_settings(
    ...
    *,
    ...
    stt_engine_id, stt_language = DefaultPipelineSettingsHelper.check_stt_engine(
        hass, pipeline_language, stt_engine_id
    )

    tts_engine_id, tts_language, tts_voice = (
        DefaultPipelineSettingsHelper.check_tts_engine(
...
class PipelineRun:
    ...
    async def wake_word_detect_words(
        self,
    ...
    ) -> tuple[wake_word.DetectionResult | None, list[EnhancedAudioChunk]]:
        """Detect wake words."""
        try:
            # Detect wake word(s)
        ...
        return result, audio_chunks_for_stt

    def handle_wake_word_detection_result(
        self,
        result: wake_word.DetectionResult | None,
        audio_chunks_for_stt: list[EnhancedAudioChunk],
    ) -> tuple[dict[str, Any], list[EnhancedAudioChunk]]:
        ...
        return wake_word_output, audio_chunks_for_stt
    ...
    async def wake_word_detection(
        ...
        result, audio_chunks_for_stt = await self.wake_word_detect_words(
            stream, wake_word_vad, stt_audio_buffer, audio_chunks_for_stt
        )

        wake_word_output, audio_chunks_for_stt = self.handle_wake_word_detection_result(
            result, audio_chunks_for_stt
        )
    ...
    async def process_volume_only(
        ...
        timestamp_ms = 0
        async for chunk in audio_stream:
            if self.audio_settings.volume_multiplier != 1:
