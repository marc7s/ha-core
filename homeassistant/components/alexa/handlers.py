...
@HANDLERS.register((Handlers.security_panel_controller, "Arm"))
async def async_api_arm(
    ...
    response = directive.response(
        name="Arm.Response",
        namespace=Handlers.security_panel_controller,
        payload=payload,
    )
...
@HANDLERS.register((Handlers.security_panel_controller, "Disarm"))
...
class SetModeHandlers:
    """Handlers for SetMode directives."""

    @staticmethod
    def fan_direction(data: dict[str, Any], mode: Any) -> str | None:
    ...
    @staticmethod
    def fan_preset_mode(
        data: dict[str, Any], mode: Any, entity: ha.State
    ) -> str | None:
    ...
    @staticmethod
    def water_heater_operation_mode(
        data: dict[str, Any], mode: Any, entity: ha.State
    ) -> str | None:
    ...
    @staticmethod
    def cover_position(mode: Any) -> str | None:
        """Set the cover position."""
        position = mode.split(".")[1]

        match position:
            case cover.STATE_CLOSED:
                return cover.SERVICE_CLOSE_COVER
            case cover.STATE_OPEN:
    ...
    @staticmethod
    def valve_position(mode: Any) -> str | None:
        """Set the valve position."""
        position = mode.split(".")[1]

        match position:
            case valve.STATE_CLOSED:
                return valve.SERVICE_CLOSE_VALVE
            case valve.STATE_OPEN:
...
async def async_api_set_mode(
    ...
    if instance == f"{fan.DOMAIN}.{fan.ATTR_DIRECTION}":
        service = SetModeHandlers.fan_direction(data, mode)

    # Fan preset_mode
    elif instance == f"{fan.DOMAIN}.{fan.ATTR_PRESET_MODE}":
        service = SetModeHandlers.fan_preset_mode(data, mode, entity)

    # Humidifier mode
    elif instance == f"{humidifier.DOMAIN}.{humidifier.ATTR_MODE}":
        service = SetModeHandlers.humidifier_mode(data, mode, entity)

    # Remote Activity
    elif instance == f"{remote.DOMAIN}.{remote.ATTR_ACTIVITY}":
        service = SetModeHandlers.remote_activity(data, mode, entity)

    # Water heater operation mode
    elif instance == f"{water_heater.DOMAIN}.{water_heater.ATTR_OPERATION_MODE}":
        service = SetModeHandlers.water_heater_operation_mode(data, mode, entity)

    # Cover Position
    elif instance == f"{cover.DOMAIN}.{cover.ATTR_POSITION}":
        service = SetModeHandlers.cover_position(mode)

    # Valve position state
    elif instance == f"{valve.DOMAIN}.state":
        service = SetModeHandlers.valve_position(mode)
...
class AdjustRangeHandlers:
    """Handlers for AdjustRangeValue directives."""
    ...
    def cover_position(
        ...
        return SERVICE_SET_COVER_POSITION, position
    ...
    def cover_tilt(
        ...
        return SERVICE_SET_COVER_TILT_POSITION, tilt_position

    @staticmethod
    def fan_speed_percentage(
        data: dict[str, Any],
        range_delta: int,
        range_delta_default: int,
        entity: ha.State,
    ) -> tuple[str, int]:
        """Adjust the fan speed percentage."""
    ...
    @staticmethod
    def humidifier_target_humidity(
        data: dict[str, Any],
        range_delta: int,
        range_delta_default: int,
        entity: ha.State,
    ) -> tuple[str, int]:
        """Adjust the humidifier target humidity."""
        percentage_step = 5
        range_delta = (
            int(range_delta * percentage_step)
            if range_delta_default
    ...
    def input_number_value(
        ...
        return input_number.SERVICE_SET_VALUE, int(value)
    ...
    def number_value(
        ...
        return number.SERVICE_SET_VALUE, int(value)
    ...
    def vacuum_fan_speed(
        ...
        speed_delta = int(range_delta)
        new_speed_index = min(
            len(speed_list) - 1, max(0, current_speed_index + speed_delta)
        )
        speed = next(
...
async def async_api_adjust_range(
    ...
    # Cover Position
    if instance == f"{cover.DOMAIN}.{cover.ATTR_POSITION}":
        service, response_value = AdjustRangeHandlers.cover_position(
            data, range_delta, range_delta_default, entity
        )

    # Cover Tilt
    elif instance == f"{cover.DOMAIN}.tilt":
        service, response_value = AdjustRangeHandlers.cover_tilt(
            data, range_delta, range_delta_default, entity
        )

    # Fan speed percentage
    elif instance == f"{fan.DOMAIN}.{fan.ATTR_PERCENTAGE}":
        service, response_value = AdjustRangeHandlers.fan_speed_percentage(
            data, range_delta, range_delta_default, entity
        )

    # Humidifier target humidity
    elif instance == f"{humidifier.DOMAIN}.{humidifier.ATTR_HUMIDITY}":
        service, response_value = AdjustRangeHandlers.humidifier_target_humidity(
            data, range_delta, range_delta_default, entity
        )

    # Input Number Value
    elif instance == f"{input_number.DOMAIN}.{input_number.ATTR_VALUE}":
        service, response_value = AdjustRangeHandlers.input_number_value(
            data, range_delta, entity
        )

    # Number Value
    elif instance == f"{number.DOMAIN}.{number.ATTR_VALUE}":
        service, response_value = AdjustRangeHandlers.number_value(
            data, range_delta, entity
        )

    # Vacuum Fan Speed
    elif instance == f"{vacuum.DOMAIN}.{vacuum.ATTR_FAN_SPEED}":
        service, response_value = AdjustRangeHandlers.vacuum_fan_speed(
            data, range_delta, entity
        )

    # Valve Position
    elif instance == f"{valve.DOMAIN}.{valve.ATTR_POSITION}":
        service, response_value = AdjustRangeHandlers.valve_position(
            data, range_delta, range_delta_default, entity
        )
